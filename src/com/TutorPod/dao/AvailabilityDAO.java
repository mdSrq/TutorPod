...
public class AvailabilityDAO {
	...
	public Availability getDuplicateWeekDay(int tutor_id,int day_of_week)throws Exception{
		Availability availability=null;
		Connection Conn = null;
		PreparedStatement Stmt = null;
		ResultSet Rs = null;
		try {
			Conn = dataSource.getConnection();
			String sql = "select * from availability where tutor_id=? and day_of_week=?";
			Stmt = Conn.prepareStatement(sql);
			Stmt.setInt(1, tutor_id);
			Stmt.setInt(2, day_of_week);
			Rs = Stmt.executeQuery();
			if(Rs.next()) {
				availability = createAvailability(Rs);
			}
	...
	public boolean addDateAvailability(Availability availability)throws Exception{
		...
		try {
			...
			Stmt = Conn.prepareStatement(sql);
			Stmt.setInt(1, availability.getTutor_id());
			Stmt.setString(2, availability.getAvail_date());
			Stmt.setString(3, availability.getTime_from());
			Stmt.setString(4, availability.getTime_to());
			if(Stmt.executeUpdate()>0)
				return true;
