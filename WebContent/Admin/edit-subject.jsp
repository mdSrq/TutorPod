    ...
    <form action="../SubjectController" method="POST" class="main__form main__form_wide" id="editSubjectForm">
        ...
        <div class="form_group">
            ...
            <input type="text" name="subject_code" id="subject_code" required
                placeholder="Enter subject code e.g. BCS012">
        </div>
...
<script type="text/javascript">
    ...
    function fetchSubjectData() {
        $.ajax({
            ...
            data: "cmd=editSubject&course_sub_id=<%=request.getParameter("
            course_sub_id ")%>",
            dataType: "json",
            processData: true,
    ...
    function fetchCourseData() {
        $.ajax({
            ...
            success: function (res) {
                $("#course_id").empty();
                $("#course_id").append($("<option>").attr({
                    selected: true,
                    disabled: true
                }).text("Select a course"));
                $("#duration_no").empty();
                $("#duration_no").append($("<option>").attr({
                    selected: true,
                    disabled: true
                }).text("Select Year/Sem"));
                var data = ""
                $.each(res, function (index, list) {
                    $("#course_id").append($("<option>").val(list.course_id).text(list.name_abbr +
                        " (" + list.course_name + ")"));
    ...
    function loadDurations() {
        ...
        $("#duration_no").append($("<option>").attr({
            selected: true,
            disabled: true
        }).text("Select " + courseDurationTypeMap.get(course_id)));
